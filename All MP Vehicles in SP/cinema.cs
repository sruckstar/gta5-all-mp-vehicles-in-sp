...
public class Cinema : Script
{
    ...
    private VehicleHash[] models = new VehicleHash[10];
    ...
    public Cinema()
    {
        ...
        models[2] = VehicleHash.Voltic2;
        models[3] = VehicleHash.Toreador;
        models[4] = VehicleHash.JB7002;
        models[5] = VehicleHash.Deluxo;
        models[6] = VehicleHash.Stromberg;
        models[7] = VehicleHash.RRocket;
        models[8] = VehicleHash.Shotaro;
        models[9] = VehicleHash.Dune5;
            ...
            for (int i = 0; i <= all_coords; i++)
            {
                if (spawned == 0 && Function.Call<bool>(Hash.IS_PLAYER_SWITCH_IN_PROGRESS) == false)
                {
                    if (Function.Call<float>(Hash.GET_DISTANCE_BETWEEN_COORDS, coords[i].X, coords[i].Y, coords[i].Z, position.X, position.Y, position.Z, 0) < distance)
                    {
                        ...
                        var veh_model = new Model(models[rnd.Next(0, 10)]);
